...
class LedFxClient:
    ...
    def update_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        response = self._request("PUT", "/api/config", config)
        self._raise_if_failed("PUT", "/api/config", response)
        return response

    def _global_brightness_from_config(self) -> Optional[float]:
        value = self.get_config().get("global_brightness")
        try:
            return max(0.0, min(1.0, float(value)))
        except (TypeError, ValueError):
            return None

    def set_global_brightness(self, value: float) -> Dict[str, Any]:
        target = max(0.0, min(1.0, float(value)))
        response = self.update_config({"global_brightness": target})
        actual = self._global_brightness_from_config()
        strategy = "patch"

        if actual is None or abs(actual - target) > 0.005:
            full_config = self.get_config()
            full_config["global_brightness"] = target
            response = self.update_config(full_config)
            actual = self._global_brightness_from_config()
        ...
        return {
            ...
            "requested_global_brightness": target,
            "strategy": strategy,
            "response": response,
        }
