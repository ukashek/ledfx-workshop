...
PRESET_BANK_FILE = DATA_DIR / "preset_bank.json"
...
class AppState:
    def __init__(self, ledfx_url: str):
        ...
        self.preset_bank = self._load_preset_bank()
    ...
    def discovery_public(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effect_profiles": self._effect_profiles_public(),
            "effect_schemas": self._effect_schemas_public(schema),
            "presets": self._presets_public(config),
            "preset_bank": self.preset_bank_public(),
            "connection": self.connection_public(),
            "global_brightness": float(config.get("global_brightness", 1.0)),
        }

    def _effect_profiles_public(self) -> Dict[str, Any]:
        result = {}
        for effect_id, profile in (self.profiles["effects"].get("effects") or {}).items():
            if not isinstance(profile, dict):
                continue
            result[str(effect_id)] = {
                "id": str(effect_id),
                "name": str(profile.get("name") or effect_id),
                "description": str(profile.get("description") or ""),
                "scene_types": list(profile.get("scene_types") or []),
                "audio_reactive": bool(profile.get("audio_reactive")),
                "reactivity_mode": str(profile.get("reactivity_mode") or ""),
                "energy": list(profile.get("energy") or []),
                "movement": profile.get("movement"),
                "rarity": profile.get("rarity"),
            }
        return result

    def show_pack_export(self) -> tuple[str, bytes]:
        created_at = time.strftime("%Y-%m-%dT%H:%M:%S%z")
        safe_slug = time.strftime("%Y%m%d-%H%M%S")
        filename = f"ledfx-workshop-show-pack-{safe_slug}.zip"
        try:
            info = self.client.get_info()
            config = self.client.get_config()
            scenes = self.client.get_scenes()
            playlists = self.client.get_playlists()
        except LedFxApiError:
            info = {}
            config = {}
            scenes = {}
            playlists = {}
        ...
        generated_scenes = [
            self.generated[scene_id].to_public_dict()
            for scene_id in self.order
            if scene_id in self.generated
        ]
        manifest = {
            "format": "ledfx-workshop-show-pack",
            "version": 1,
            ...
            "ledfx": {
                "info": info,
                "user_presets": config.get("user_presets", {}),
        ...
        with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            archive.writestr("show_pack.json", json.dumps(manifest, indent=2))
            archive.writestr("profiles/effects.json", json.dumps(self.profiles.get("effects", {}), indent=2))
            archive.writestr("profiles/palettes.json", json.dumps(self.profiles.get("palettes", {}), indent=2))
            archive.writestr("profiles/styles.json", json.dumps(self.profiles.get("styles", {}), indent=2))
            archive.writestr("profiles/preset_bank.json", json.dumps(self.preset_bank_public(), indent=2))
            archive.writestr("ledfx/scenes.json", json.dumps(manifest["ledfx"]["scenes"], indent=2))
            archive.writestr("ledfx/playlists.json", json.dumps(manifest["ledfx"]["playlists"], indent=2))
            archive.writestr("ledfx/user_presets.json", json.dumps(manifest["ledfx"]["user_presets"], indent=2))
            archive.writestr(
                "README.txt",
                "\n".join(
                    [
                        "LedFx Workshop Show Pack",
                        "",
                        "This ZIP contains Workshop profiles, the local generated scene queue,",
                        "and exported LedFx scenes/playlists/user presets for backup or transfer.",
                        "",
                        "Import it from LedFx Workshop using Import Show Pack.",
                    ]
                ),
            )
        return filename, buffer.getvalue()

    def import_show_pack(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        raw = str(payload.get("archive") or payload.get("data") or "").strip()
        if not raw:
        ...
        try:
            ...
            raise ValueError("archive must be base64 encoded") from exc

        manifest = self._read_show_pack_manifest(content)
        if manifest.get("format") != "ledfx-workshop-show-pack":
            raise ValueError("Unsupported show pack format")
        workshop = manifest.get("workshop") or {}
        profiles = workshop.get("profiles") or {}
        imported_profiles = []
        if payload.get("import_profiles", True) and isinstance(profiles, dict):
            for key in ("effects", "palettes", "styles"):
                if isinstance(profiles.get(key), dict):
                    self.profiles[key] = profiles[key]
                    imported_profiles.append(key)
        ...
        imported_scenes = []
        if payload.get("import_queue", True):
            scenes = workshop.get("generated_scenes") or []
            ...
            for item in scenes:
                if not isinstance(item, dict):
                    continue
                scene = self._scene_from_public(item)
                self.generated[scene.id] = scene
                self.order.append(scene.id)
                imported_scenes.append(scene.id)
            self.last_options = dict(workshop.get("last_options") or {})
            generator = self._generator()
            active_scenes = [self.generated[sid] for sid in self.order if sid in self.generated and not self.generated[sid].deleted]
            self.last_similarity_report = generator.similarity_report(active_scenes) if active_scenes else {}

        preset_bank = workshop.get("preset_bank") or {}
        if payload.get("import_preset_bank", True) and isinstance(preset_bank, dict):
            imported_items = preset_bank.get("items") or []
            if isinstance(imported_items, list):
                existing = {str(item.get("id")): item for item in self.preset_bank.get("items", []) if isinstance(item, dict)}
                for item in imported_items:
                    if isinstance(item, dict) and item.get("id"):
                        existing[str(item["id"])] = item
                self.preset_bank = {"items": list(existing.values())}
                self._write_preset_bank()

        return {
            "ok": True,
            "profiles": imported_profiles,
            "generated_scene_count": len(imported_scenes),
            "ledfx_scene_count": len((manifest.get("ledfx") or {}).get("scenes") or {}),
            "playlist_count": len((manifest.get("ledfx") or {}).get("playlists") or {}),
            "preset_bank_count": len(self.preset_bank.get("items", [])),
            "scenes": [self.generated[sid].to_public_dict() for sid in self.order if sid in self.generated],
            "similarity_report": self.last_similarity_report,
        }

    def _read_show_pack_manifest(self, content: bytes) -> Dict[str, Any]:
        try:
            with zipfile.ZipFile(io.BytesIO(content)) as archive:
                with archive.open("show_pack.json") as handle:
                    return json.loads(handle.read().decode("utf-8"))
        except zipfile.BadZipFile:
            return json.loads(content.decode("utf-8"))

    @staticmethod
    def _scene_from_public(item: Dict[str, Any]) -> Scene:
        assignments = []
        for assignment in item.get("assignments") or []:
            ...
            assignments.append(
                VirtualAssignment(
                    virtual_id=str(assignment.get("virtual_id") or ""),
                    effect_type=str(assignment.get("effect_type") or ""),
                    config=copy.deepcopy(assignment.get("config") or {}),
                    preset=assignment.get("preset"),
                    preset_category=assignment.get("preset_category"),
                    source_preset_id=assignment.get("source_preset_id"),
                    source_preset_name=assignment.get("source_preset_name"),
                    source_preset_category=assignment.get("source_preset_category"),
                    action=str(assignment.get("action") or "activate"),
                )
            )
        return Scene(
            id=str(item.get("id") or f"imported-{int(time.time())}"),
            name=str(item.get("name") or "Imported Scene")[:MAX_SCENE_NAME],
            scene_type=str(item.get("scene_type") or "scene"),
            style=str(item.get("style") or "imported"),
            palette_id=str(item.get("palette_id") or "auto"),
            palette_name=str(item.get("palette_name") or item.get("palette_id") or "Auto"),
            energy=float(item.get("energy") or 0.5),
            layout=str(item.get("layout") or "unison"),
            assignments=assignments,
            tags=list(item.get("tags") or []),
            mood_tags=list(item.get("mood_tags") or []),
            fingerprint=list(item.get("fingerprint") or []),
            kept=bool(item.get("kept", True)),
            deleted=bool(item.get("deleted", False)),
            saved=bool(item.get("saved", False)),
            ledfx_scene_id=item.get("ledfx_scene_id"),
        )
    ...
    def preset_bank_public(self) -> Dict[str, Any]:
        items = []
        for item in self.preset_bank.get("items", []):
            if not isinstance(item, dict):
                continue
            clean = self._sanitize_preset_bank_item(item, validate_schema=False)
            if clean:
                items.append(clean)
        items.sort(key=lambda item: (item.get("effect_type", ""), item.get("name", "").lower(), item.get("id", "")))
        return {"items": items, "count": len(items)}

    def save_preset_bank_item(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        schema_effects = self.client.get_schema().get("effects", {})
        item = self._sanitize_preset_bank_item(payload, schema_effects=schema_effects)
        if not item:
            raise ValueError("Preset bank item is required")
        items = [entry for entry in self.preset_bank.get("items", []) if isinstance(entry, dict)]
        index = next((idx for idx, entry in enumerate(items) if str(entry.get("id")) == item["id"]), -1)
        if index >= 0:
            created_at = items[index].get("created_at") or item["created_at"]
            item["created_at"] = created_at
            items[index] = item
        else:
            items.append(item)
        self.preset_bank = {"items": items}
        self._write_preset_bank()
        return {"item": item, "preset_bank": self.preset_bank_public()}

    def delete_preset_bank_item(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        item_id = str(payload.get("id") or payload.get("preset_id") or "").strip()
        if not item_id:
            raise ValueError("preset bank id is required")
        items = [entry for entry in self.preset_bank.get("items", []) if isinstance(entry, dict)]
        next_items = [entry for entry in items if str(entry.get("id")) != item_id]
        if len(next_items) == len(items):
            raise KeyError(f"Unknown preset bank item: {item_id}")
        self.preset_bank = {"items": next_items}
        self._write_preset_bank()
        return {"deleted": item_id, "preset_bank": self.preset_bank_public()}

    def preset_bank_from_scene(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        scene = self._scene(str(payload.get("scene_id") or ""))
        assignment_index = payload.get("assignment_index")
        assignment = None
        if assignment_index is not None:
            try:
                candidate = scene.assignments[int(assignment_index)]
                if candidate.effect_type:
                    assignment = candidate
            except (IndexError, TypeError, ValueError):
                assignment = None
        if assignment is None:
            assignment = next((item for item in scene.assignments if item.effect_type and item.action != "ignore"), None)
        if assignment is None:
            raise ValueError("Scene has no active effect to save as a preset bank item")
        payload = {
            "name": str(payload.get("name") or f"{scene.name} {assignment.effect_type}")[:64],
            "effect_type": assignment.effect_type,
            "config": copy.deepcopy(assignment.config),
            "palette_id": scene.palette_id,
            "palette_name": scene.palette_name,
            "tags": self._merge_scene_tags(scene.tags, scene.mood_tags + ["preset-bank"]),
            "source": "Scene",
            "source_scene_id": scene.id,
            "source_scene_name": scene.name,
            "source_virtual_id": assignment.virtual_id,
        }
        return self.save_preset_bank_item(payload)

    def _sanitize_preset_bank_item(
        self,
        item: Dict[str, Any],
        schema_effects: Optional[Dict[str, Any]] = None,
        validate_schema: bool = True,
    ) -> Optional[Dict[str, Any]]:
        if not isinstance(item, dict):
            return None
        effect_type = str(item.get("effect_type") or "").strip()
        name = str(item.get("name") or item.get("id") or "").strip()[:64]
        config_data = item.get("config") or {}
        if not effect_type or not name or not isinstance(config_data, dict):
            if validate_schema:
                raise ValueError("name, effect_type and config are required")
            return None
        if validate_schema and schema_effects is not None:
        ...
        if not item_id:
            ...
            item_id = f"bank-{self._slug(name)}-{digest}"
        palette = self._palette_from_config(config_data)
        palette_id = str(item.get("palette_id") or (palette or {}).get("id") or "").strip()
        palette_name = str(item.get("palette_name") or (palette or {}).get("name") or palette_id or "").strip()
        now = time.strftime("%Y-%m-%dT%H:%M:%S%z")
        updated_at = now if validate_schema else str(item.get("updated_at") or now)
        tags = self._merge_scene_tags(item.get("tags"), ["preset-bank", effect_type])
        if palette_id:
            tags = self._merge_scene_tags(tags, [f"palette-{palette_id}"])
        return {
            "id": item_id[:96],
            "name": name,
            "effect_type": effect_type,
            "source": str(item.get("source") or "Preset Bank"),
            "preset_id": str(item.get("preset_id") or ""),
            "preset_category": str(item.get("preset_category") or ""),
            ...
            "source_scene_name": str(item.get("source_scene_name") or ""),
            "source_virtual_id": str(item.get("source_virtual_id") or ""),
        }

    def _palette_from_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        gradients = [
            value
            for key, value in (config or {}).items()
            if isinstance(value, str) and "linear-gradient" in value and "gradient" in str(key).lower()
        ]
        if not gradients:
            return None
        normalized = self._normalize_gradient(gradients[0])
        for palette in self.profiles["palettes"].get("palettes", []):
            if not isinstance(palette, dict):
                continue
            try:
                gradient = palette.get("gradient") or self._palette_gradient(
                    palette.get("colors") or {},
                    palette.get("positions") or None,
                )
            except Exception:
                continue
            if self._normalize_gradient(gradient) == normalized:
                return palette
        return None
    ...
    def _generator(self) -> SceneGenerator:
        ...
        return SceneGenerator(
            ...
            schema=schema,
            ledfx_config=config,
            preset_bank_items=self.preset_bank_public().get("items", []),
        )
    ...
    def manual_scene(self, options: Dict[str, Any]) -> Dict[str, Any]:
        options = self._normalize_options(options)
        options["count"] = 1
        options["existing_scenes"] = [
            self.generated[sid]
            for sid in self.order
            if sid in self.generated and not self.generated[sid].deleted
        ]
        generator = self._generator()
        scene = generator.generate_batch(options)[0]
        scene.tags = self._merge_scene_tags(scene.tags, ["manual"])
        self.generated[scene.id] = scene
        self.order.append(scene.id)
        self.last_options = dict(options)
        active_scenes = [
    ...
    def delete_generated(self, scene_id: str) -> Dict[str, Any]:
        ...
        scene.deleted = True
        scene.kept = False
        report = self._refresh_similarity_report()
        return {"scene": scene.to_public_dict(), "similarity_report": report}

    def diversify(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        base_options = dict(self.last_options)
        options = payload.get("options")
        if isinstance(options, dict):
            base_options.update(options)
        report = self.last_similarity_report or self._refresh_similarity_report()
        requested_ids = payload.get("scene_ids")
        if isinstance(requested_ids, list):
            candidate_ids = [str(scene_id).strip() for scene_id in requested_ids if str(scene_id).strip()]
        else:
            candidate_ids = [
                str(scene_id)
                for scene_id in report.get("regenerate_candidate_ids", [])
                if str(scene_id)
            ]
        if not candidate_ids:
            for pair in report.get("pairs", []):
                if pair.get("level") not in ("high", "medium"):
                    continue
                for key in ("right_id", "left_id"):
                    scene_id = str(pair.get(key) or "")
                    if scene_id and scene_id not in candidate_ids:
                        candidate_ids.append(scene_id)
                if len(candidate_ids) >= 8:
                    break
        candidate_ids = [
            scene_id
            for scene_id in self._ordered_unique_ids(candidate_ids)[:24]
            if scene_id in self.generated and not self.generated[scene_id].deleted
        ]
        if not candidate_ids:
            return {
                "scenes": self._generated_scene_payload(),
                "replaced": [],
                "similarity_report": report,
                "preset_errors": [],
            }
        ...
        base_options["count"] = 1
        base_options["variation"] = max(
            self._bounded_float(base_options.get("variation"), DEFAULT_STYLE_PARAMS["variation"]),
            0.78,
        )
        generator = self._generator()
        replaced = []
        for old_id in candidate_ids:
            ...
            replaced.append(
                {
                    "old_id": old_id,
                    "old_name": old_scene.name,
                    "new_id": replacement.id,
                    "new_name": replacement.name,
                }
            )
        self.last_options = dict(base_options)
        self.last_options.pop("existing_scenes", None)
        report = self._refresh_similarity_report()
        return {
            "scenes": self._generated_scene_payload(),
            "replaced": replaced,
    ...
    def _active_generated_scenes(self) -> List[Scene]:
        return [
            self.generated[scene_id]
            for scene_id in self.order
            if scene_id in self.generated and not self.generated[scene_id].deleted
        ]

    def _generated_scene_payload(self) -> List[Dict[str, Any]]:
        return [
            self.generated[scene_id].to_public_dict()
            for scene_id in self.order
            if scene_id in self.generated
    ...
    def _refresh_similarity_report(self) -> Dict[str, Any]:
        active_scenes = self._active_generated_scenes()
        self.last_similarity_report = SceneScorer().batch_report(active_scenes) if active_scenes else {}
        return self.last_similarity_report

    def validate_batch(self, scene_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        return self._validate_scenes_for_ledfx(scenes)

    def _validate_scenes_for_ledfx(self, scenes: List[Scene]) -> Dict[str, Any]:
        schema = self.client.get_schema()
        config = self.client.get_config()
        virtuals = self.client.get_virtuals().get("virtuals") or {}
        known_virtual_ids = set(virtuals.keys())
        schema_effects = schema.get("effects") or {}
        errors: List[Dict[str, str]] = []
        warnings: List[Dict[str, str]] = []
        for scene in scenes:
            seen_virtuals = set()
            if not scene.assignments:
                errors.append({"scene": scene.name, "field": "virtuals", "message": "Scene has no device assignments."})
            for assignment in scene.assignments:
                virtual_id = assignment.virtual_id
                if not virtual_id:
                    errors.append({"scene": scene.name, "field": "device", "message": "Missing device id."})
                    continue
                if virtual_id in seen_virtuals:
                    errors.append({"scene": scene.name, "field": "device", "message": f"Device {virtual_id} is assigned more than once."})
                seen_virtuals.add(virtual_id)
                if virtual_id not in known_virtual_ids:
                    errors.append({"scene": scene.name, "field": "device", "message": f"Unknown LedFx device: {virtual_id}."})
                action = self._sanitize_scene_action(assignment.action, "activate")
                if action == "ignore":
                    continue
                if assignment.effect_type not in schema_effects:
                    errors.append({"scene": scene.name, "field": "effect", "message": f"Unknown LedFx effect: {assignment.effect_type}."})
                    continue
                properties = (
                    schema_effects.get(assignment.effect_type, {})
                    .get("schema", {})
                    .get("properties", {})
                )
                if isinstance(properties, dict) and properties:
                    ...
                    if unknown:
                        warnings.append(
                            {
                                "scene": scene.name,
                                "field": assignment.effect_type,
                                "message": f"Unknown config keys ignored by safety check: {', '.join(unknown[:8])}.",
                            }
                        )
                if assignment.preset:
                    category = assignment.preset_category or "ledfx_presets"
                    exists = (
                        (config.get(category) or {})
                        .get(assignment.effect_type, {})
                        .get(assignment.preset)
                    )
                    if not exists and category != "draft_presets":
                        warnings.append(
                            {
                                "scene": scene.name,
                                "field": "preset",
                                "message": f"Preset {assignment.preset} was not found for {assignment.effect_type}; config will still be sent.",
                            }
                        )
        return {
            "ok": not errors,
    ...
    def _preset_details(
        ...
        if not preset_id:
            return None
        if category == "preset_bank":
            return self._preset_bank_details(effect_type, preset_id)
    ...
    def _preset_bank_details(self, effect_type: str, preset_id: str) -> Optional[Dict[str, Any]]:
        for item in self.preset_bank_public().get("items", []):
            if (
                str(item.get("id") or "") == str(preset_id)
                and str(item.get("effect_type") or "") == str(effect_type)
                ...
                return {
                    ...
                    "palette_name": str(item.get("palette_name") or ""),
                    "tags": self._scene_tags_list(item.get("tags")),
                }
        return None
    ...
    def update_scene(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for assignment, target_virtual_id, action, effect_type, preset, preset_requested, config_update in resolved_updates:
            if effect_type != assignment.effect_type:
                ...
                assignment.source_preset_id = None
            ...
            if action != assignment.action:
                ...
                changed = True
            if preset:
                preset_config = copy.deepcopy(preset["config"])
                if preset["category"] == "preset_bank":
                    if (
                        assignment.source_preset_id != preset["id"]
                        or assignment.source_preset_category != "preset_bank"
                    ):
                        changed = True
                    assignment.preset = None
                    assignment.preset_category = None
                    assignment.source_preset_id = preset["id"]
                    assignment.source_preset_name = preset["name"]
                    assignment.source_preset_category = "preset_bank"
                else:
                ...
                if assignment.config != preset_config:
                    assignment.config = preset_config
                    changed = True
            elif preset_requested:
                if assignment.preset or assignment.source_preset_id:
                    assignment.preset = None
                    assignment.preset_category = None
                    assignment.source_preset_id = None
                    assignment.source_preset_name = None
        ...
        if changed:
            scene.saved = False
            self._refresh_scene_metadata(scene)
            report = self._refresh_similarity_report()
        else:
            report = self.last_similarity_report
        return {"scene": scene.to_public_dict(), "similarity_report": report}
    ...
    def save_batch(self, scene_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for scene in selected_scenes:
            self._refresh_scene_metadata(scene)
        safety_report = self._validate_scenes_for_ledfx(selected_scenes)
        if not safety_report["ok"]:
            return {
                ...
                "errors": [
                    {
                        "id": "",
                        "name": item.get("scene", "Scene"),
                        "error": item.get("message", "Scene failed LedFx safety validation."),
                    }
                    for item in safety_report["errors"]
                ],
                "preset_errors": [],
        ...
        return {"saved": saved, "errors": errors, "preset_errors": preset_errors, "safety_report": safety_report}
    ...
    def set_global_brightness(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        value = self._normalize_global_brightness(payload.get("global_brightness", payload.get("value")))
        response = self.client.set_global_brightness(value)
        actual = self._bounded_float(response.get("global_brightness"), value, low=0.0, high=1.0)
        return {
            "global_brightness": actual,
            "requested_global_brightness": value,
    ...
    def save_style(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        clean["scene_type_weights"] = self._sanitize_weight_map(
            payload.get("scene_type_weights"),
            clean.get("scene_type_weights"),
            list(self.profiles["styles"].get("scene_types", {}).keys()),
        )
        clean["effect_bias"] = self._sanitize_weight_map(
            payload.get("effect_bias"),
            clean.get("effect_bias"),
            list(self.profiles["effects"].get("effects", {}).keys()),
        )
        clean["palette_bias"] = self._sanitize_weight_map(
            payload.get("palette_bias"),
            clean.get("palette_bias"),
            [
                str(palette.get("id"))
                for palette in self.profiles["palettes"].get("palettes", [])
                if palette.get("id")
    ...
    def _sync_config_gradient_name(self, config: Dict[str, Any], config_update: Dict[str, Any]) -> None:
        ...
        for key, value in config.items():
            if (
                ...
                and "linear-gradient" in value
                and "gradient" in str(key).lower()
            ):
                gradient_key = str(key)
                gradient = value
                break
        ...
        elif gradient_key in config_update and "gradient_name" not in config_update:
    ...
    def _refresh_scene_metadata(self, scene: Scene) -> None:
        palette = self._scene_palette_from_assignments(scene)
        if palette:
            scene.palette_id = str(palette.get("id") or scene.palette_id or "")
            scene.palette_name = str(palette.get("name") or scene.palette_name or scene.palette_id)
        mood_tags = self._infer_scene_mood_tags(scene)
        protected_tags = [
            tag
            for tag in self._scene_tags_list(scene.tags)
            if not tag.startswith("mood-") and not tag.startswith("palette-")
        ]
        palette_tags = [f"palette-{scene.palette_id}"] if scene.palette_id and scene.palette_id != "auto" else []
        scene.mood_tags = mood_tags
        scene.tags = self._merge_scene_tags(protected_tags, palette_tags + mood_tags)
        scene.fingerprint = SceneScorer().fingerprint(scene)

    def _scene_palette_from_assignments(self, scene: Scene) -> Optional[Dict[str, Any]]:
        for assignment in scene.assignments:
            if assignment.action == "ignore" or not isinstance(assignment.config, dict):
                continue
            palette = self._palette_from_config(assignment.config)
            if palette:
                return palette
        return None

    def _infer_scene_mood_tags(self, scene: Scene) -> List[str]:
        tags: List[str] = []
        scene_type_tags = {
        ...
        tags.extend(scene_type_tags.get(scene.scene_type, []))
        if scene.energy >= 0.82:
            tags.append("mood-intense")
        elif scene.energy <= 0.34:
            tags.append("mood-low")
        effect_text = " ".join(
            assignment.effect_type
            for assignment in scene.assignments
            if assignment.effect_type and assignment.action != "ignore"
        ).lower()
        if any(token in effect_text for token in ("strobe", "flash", "sparkle")):
            tags.append("mood-flash")
        if any(token in effect_text for token in ("bar", "meter", "magnitude")):
            tags.append("mood-meter")
        if any(token in effect_text for token in ("scroll", "blade", "scan", "wavelength")):
            tags.append("mood-flow")
        if any(token in effect_text for token in ("bass", "power", "pitch")):
            tags.append("mood-bass")
        if scene.layout in ("mirror", "alternate"):
            tags.append("mood-wide")
        elif scene.layout == "accent":
            tags.append("mood-accent")
        palette_text = f"{scene.palette_id} {scene.palette_name}".lower()
        if any(token in palette_text for token in ("warm", "red", "thermal", "sun", "amber")):
            tags.append("mood-warm")
        if any(token in palette_text for token in ("cold", "blue", "steel", "subzero", "sodium")):
            tags.append("mood-cold")
        if any(token in palette_text for token in ("neon", "magenta", "acid", "grime")):
            tags.append("mood-neon")
        style_text = str(scene.style or "").lower()
        if "dub" in style_text:
            tags.append("mood-dub")
        if "grime" in style_text:
            tags.append("mood-grime")
        return self._ordered_unique_ids(tags)[:6]
    ...
    def _normalize_options(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        preset_bank_mode = str(clean.get("preset_bank_mode") or "assist").strip()
        clean["preset_bank_mode"] = preset_bank_mode if preset_bank_mode in ("off", "assist", "prefer") else "assist"
    ...
    @staticmethod
    def _normalize_global_brightness(value: Any) -> float:
        try:
            numeric = float(value)
        except (TypeError, ValueError):
            return 1.0
    ...
    def _sanitize_weight_map(
        ...
        for key in allowed_keys:
            raw = source.get(key, 1.0)
            try:
                weight = float(raw)
            except (TypeError, ValueError):
                weight = 1.0
            clean[key] = round(max(low, min(high, weight)), 3)
    ...
    @staticmethod
    def _load_preset_bank() -> Dict[str, Any]:
        if not PRESET_BANK_FILE.exists():
            return {"items": []}
        try:
            loaded = json.loads(PRESET_BANK_FILE.read_text(encoding="utf-8"))
        except Exception:
            return {"items": []}
    ...
    def _write_preset_bank(self) -> None:
        PRESET_BANK_FILE.write_text(json.dumps(self.preset_bank_public(), indent=2) + "\n", encoding="utf-8")
    ...
    def _write_effects(self) -> None:
        ...
        body = json.dumps(self.profiles["effects"], indent=2) + "\n"
        path.write_text(body, encoding="utf-8")
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_api_get(self) -> None:
        try:
            if self.path == "/api/show-pack/export":
                filename, content = self.state.show_pack_export()
                self._binary(200, content, "application/zip", filename)
                return
    ...
    def _handle_api_post(self) -> None:
        try:
            ...
            if self.path == "/api/generate":
                ...
                return
            if self.path == "/api/manual-scene":
                self._json(200, self.state.manual_scene(body))
                return
            if self.path == "/api/validate-batch":
                self._json(200, self.state.validate_batch(body.get("scene_ids")))
                return
            if self.path == "/api/show-pack/import":
                self._json(200, self.state.import_show_pack(body))
                return
            if self.path == "/api/regenerate":
                self._json(200, self.state.regenerate(body["scene_id"], body.get("options")))
                return
            if self.path == "/api/diversify":
                self._json(200, self.state.diversify(body))
                return
            ...
            if self.path == "/api/presets/preview":
                ...
                return
            if self.path == "/api/preset-bank/save":
                self._json(200, self.state.save_preset_bank_item(body))
                return
            if self.path == "/api/preset-bank/delete":
                self._json(200, self.state.delete_preset_bank_item(body))
                return
            if self.path == "/api/preset-bank/from-scene":
                self._json(200, self.state.preset_bank_from_scene(body))
                return
            ...
            if self.path == "/api/playlists/control":
                ...
                return
            if self.path == "/api/ledfx/global-brightness":
                self._json(200, self.state.set_global_brightness(body))
                return
    ...
    def _binary(self, status: int, content: bytes, content_type: str, filename: str) -> None:
        self.send_response(status)
        self.send_header("Content-Type", content_type)
